...
class FMUApp(TFTApp):
	...
	def set_states(self,app,menu,sub):
		...
		menu_max = len(self.sub_menus) - 1
		if menu > menu_max:
			menu = 0
		elif menu < 0:
			menu = menu_max
			
		sub_max = len( self.sub_menus[ self.menu_state ][ 'tracks' ] ) - 1
		if sub > sub_max:
			sub = 0
		elif sub < 0:
			sub = sub_max
	...
	def do_mpc_control(self):
		title = self.sub_menus[self.menu_state]['tracks'][self.sub_menu_state]['title']
		if  title == 'play_pause':
	...
	def do_function(self):
		if self.sub_menus[self.menu_state]['tracks'][self.sub_menu_state]['title'] == 'restart':
			self.do_restart()
	
	def load_archive(self):
		url = self.sub_menus[self.menu_state]['tracks'][self.sub_menu_state]['url']
	...
	def render_main_menu(self):
		...
		for i in range( len(self.sub_menus) - 1 ):
			option_name = self.sub_menus[i+1]['title']
	...
	def render_sub_menu(self):
		...
		if self.sub_menus[self.menu_state]['title'] == 'archives' and self.got_archives == False:
			self.get_archives()
		
		options_per_page = 5
		page = int(self.sub_menu_state / options_per_page)
		active_index = self.sub_menu_state % options_per_page
		sub_menu = self.sub_menus[self.menu_state]['tracks']
	...
	def update_surface(self):
		...
		if self.screensaver_on == False:
			...
			self.surface.blit(self.screensaver.surface, (0,30))
		return self.surface
...
class MPC:
	...
	def startListener(self):
		try:
			thread.start_new_thread( self.get_stats, ())
		except:
...
class TFT:
	...
	def __init__(self):
		...
		self.screen = pygame.display.set_mode((160, 128), 0, 32)
		self.background = pygame.Surface(self.screen.get_size())
		self.background.convert()
		
		self.fmu = FMUApp(self.screen.get_size())
		self.camera = CameraApp(self.screen.get_size())
		self.apps = [self.fmu, self.camera]
